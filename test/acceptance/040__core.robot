...
Unexpected Fault
    Create Soap Client    ${TEST WSDL URL}
    Run Keyword And Expect Error    WebFault: Server raised fault: 'fault message'    Call Soap Method    fault    fault message
...
Get Last Sent/Received
    ...
    Run Keyword And Expect Error    The SudsLibrary SOAP logging message plugin has been removed.    Get Last Sent
...
Default Socket Timeout Is Restored
    [Documentation]    When running in an interpreter with version < 2.6, suds changes the default socket timeout. Make sure it is restored so as to not interfere with other network code.
    [Setup]    Save And Set Global Timeout and urllib2 Version
    Create Soap Client    ${TEST WSDL URL}
    ...
    Default Socket Timeout Should Be    ${42}
    [Teardown]    Restore Global Timeout And urllib2 Version

Timeout Set On Create
    Create Soap Client    ${TEST WSDL URL}    timeout=45 sec
    ${sl}=    Get Library Instance    SudsLibrary
...
Restore Global Timeout And urllib2 Version
    ${sl}=    Get Library Instance    SudsLibrary
    Log    ${sl._global_timeout}    # sanity check
    ${sl._global_timeout}=    Set Variable    ${old global timeout}
    ${u2}=    Evaluate    urllib2    urllib2
    ${u2.__version__}=    Set Variable    ${old u2ver}

Save And Set Global Timeout and urllib2 Version
    ${sl}=    Get Library Instance    SudsLibrary
    Log    ${sl._global_timeout}    # sanity check
    Set Test Variable    ${old global timeout}    ${sl._global_timeout}
    ${sl._global_timeout}=    Set Variable    ${True}
    ${u2}=    Evaluate    urllib2    urllib2
    Set Test Variable    ${old u2ver}    ${u2.__version__}
    ${u2.__version__}=    Set Variable    2.5
